...
import { initDisplay, setStep, setStatus, setHeard, type EvenBridge } from './display'
...
function getStatusClass(): string {
  if (previewState.status.startsWith('✓ CONFIRMED') || previewState.status.startsWith('✓ ALL')) return 'confirmed'
  if (previewState.status.startsWith('↺')) return 'repeat'
  if (previewState.status.startsWith('⚑')) return 'flagged'
  return 'listening'
}
...
async function applyStatus(status: import('./display').DisplayStatus) {
  previewState.status = {
...
async function onSpeech(text: string) {
  ...
  if (awaitingFlagResume) return  // waiting for tap, ignore speech
  processing = true
...
async function main() {
  ...
  bridge.onEvenHubEvent((event: Parameters<typeof bridge.onEvenHubEvent>[0] extends (e: infer E) => unknown ? E : never) => {

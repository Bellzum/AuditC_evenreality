...
export type VerificationOutcome = 'confirmed' | 'repeat' | 'flagged'
...
export function classifyText(text: string): VerificationOutcome {
  const lower = text.toLowerCase()
  if (FLAG_TRIGGERS.some(t => lower.includes(t))) return 'flagged'
  if (EXCEPTION_WORDS.some(w => lower.includes(w))) return 'repeat'
  return 'confirmed'
}
